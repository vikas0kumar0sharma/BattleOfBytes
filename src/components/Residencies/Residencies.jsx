    ...
    <div className="r-wrapper" id='all-teams'>
      <div className="paddings innerWidth r-container">
        ...
        <span className='orangeText'>Battle Of Bytes <br/> <hr style={{width:'75%',margin:'auto'}} /> </span>
          <span className='primaryText'>All Teams and Bidders</span>
        </div>
            ...
            data.map((it, index) => (
              <SwiperSlide key={index}>
                <div className="flexColStart r-card" id={it.name}>
                  <img src={it.image} alt='home' style={{height:'8rem',width:'8rem'}}></img>
                  <span className="secondaryText r-price">
                    <span> <span style={{ color: 'orange' }}></span> {it.mentor}</span>
                  </span>
                  <span className='primaryText' style={{ fontSize: '1.5rem' }}>{it.name}</span>
                  <span className='secondaryName' style={{ fontSize: '0.7rem', width: '15rem', color: 'grey' }}>{it.slogan}</span>
                </div>
              </SwiperSlide>
            ))
          }
